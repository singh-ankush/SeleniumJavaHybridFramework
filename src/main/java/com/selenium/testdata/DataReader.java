...
public class DataReader extends TestBase {
    ...
    //multiple data
    public static HashMap<String, List<HashMap<String, String>>> testDataMappedWithMultipleRows(String fileName, String sheetName) {
        HashMap<String, List<HashMap<String, String>>> testData = new HashMap<>();

        try {
            XSSFWorkbook workbook = setExcelWorkbook(fileName);
            XSSFSheet sheet = setExcelSheet(workbook, sheetName);

            int rowCount = getRowCount(sheet);
            int columnCount = getColumnCount(sheet);
            String[] headers = getColumnHeaders(sheet);

            for (int i = 1; i < rowCount; i++) {
                HashMap<String, String> rowData = new HashMap<>();
                for (int j = 0; j < columnCount; j++) {
                    rowData.put(headers[j], getCellData(i, j, sheet));
                }

                String testCaseName = rowData.get("TestCaseName");

                // Only add if Runmode is 'Y'
                if (rowData.get("Runmode").equalsIgnoreCase("Y")) {
                    testData.computeIfAbsent(testCaseName, k -> new ArrayList<>()).add(rowData);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
